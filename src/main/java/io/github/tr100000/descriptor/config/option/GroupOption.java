package io.github.tr100000.descriptor.config.option;
        ...
        if (mapLike.isSuccess()) {
            ...
            for (AbstractOption<?> option : options) {
                if (map.get(option.getName()) != null)
                    ...
                    Descriptor.LOGGER.warn("Map didn't contain value for \"{}\"", option.getName());
            }
        }
        else {
            Descriptor.LOGGER.warn("Failed to decode group \"{}\", will revert to default values", getName());
        }
    }
}
